...
// Check for 'id' parameter in GET request
if (isset($_GET['id'])) {
    $id = $_GET['id'];

    // Prepare SQL statement to fetch portfolio details by industry ID
    $statement = $pdo->prepare("
        ...
        WHERE p.status = 1 AND p.ind_id = ?
        ORDER BY p.p_id DESC
    ");
    $statement->execute([$id]);
    $portfolios = $statement->fetchAll(PDO::FETCH_ASSOC);

    // If no portfolio found, redirect
    if (!$portfolios) {
                ...
                <div class="mixitup-gallery">
                    ...
                    <div class="filters centered clearfix">
                        <ul class="filter-tabs filter-btns clearfix">
                            ...
                            foreach ($states as $state) : ?>
                                <li class="filter" data-role="button" data-filter=".<?= $state['state_id']; ?>">
                                    <?= $state['state_capital']; ?>
                                </li>
                            <?php endforeach; ?>
                    ...
                    <div class="filter-list row">
                        <?php foreach ($portfolios as $portfolio) : ?>
                            <?php $state_class = $portfolio['state_id']; ?>
