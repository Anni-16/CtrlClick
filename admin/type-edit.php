...
if (isset($_POST['form1'])) {
    ...
    if (empty($_POST['type_name'])) {
        ...
        $statement = $pdo->prepare("SELECT * FROM tbl_type WHERE type_name=? AND type_id!=?");
        $statement->execute(array($_POST['type_name'], $id));
    ...
    if ($valid == 1) {
        ...
        $statement = $pdo->prepare("UPDATE tbl_type SET type_name=?, status=? WHERE type_id=?");
        $statement->execute(array($_POST['type_name'], $_POST['status'], $id));

        // Generate slug and update URL
        $slug_url = preg_replace('/[^a-z0-9]+/i', '-', strtolower($type_name));
        $slug_url = rtrim($slug_url, '-');

        $stmt2 = $pdo->prepare("UPDATE tbl_type SET url = ? WHERE type_id = ?");
        $stmt2->execute([$slug_url, $id]);

        $success_message = 'Type Name updated successfully.';
    }
}
?>
